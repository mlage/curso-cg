...
class GPUCanvas {
    ...
    /**
     * Cria os recursos mais comuns de um uniforme: dados, buffer e bind group.
     */
    createUniform(program, data, binding = 0, groupIndex = 0) {
        const source = toTypedArray(data);
        const buffer = this.createUniformBuffer(source);
        const bindGroup = this.createUniformBindGroup(program, buffer, binding, groupIndex);
